...
use sandbox_steward::{CommitKind, RecordKind, Steward, StewardOptions};
...
proptest! {
        ...
        block_on(async move {
            ...
            for seq in 1..=last {
                ...
                if has_dc {
                    ...
                    match result {
                        Err(sandbox_remote::RemoteError::NoSuchCommit(s)) => {
    ...
    fn prop_invariants_hold_throughout_random_long_sequence(
        ...
        block_on(async move {
            ...
            let _ = remote.pull(&mut consumer).await.unwrap();
            let report = sandbox_steward::verify_local(&consumer).await.unwrap();
