...
use sandbox_remote::chunking::assemble_file;
use sandbox_remote::schema::{
    PARTITION_KIND_CHECKSUM, PARTITION_KIND_DATA, RemoteRow, RowBody, record_batch_to_rows,
};
use sandbox_steward::{CommitKind, RecordKind, Steward};
...
async fn partition_checksums_round_trip_byte_exact() {
    ...
    let mut remote_kinds: HashMap<String, sandbox_store::checksum::ChecksumKind> = HashMap::new();
...
async fn data_rows_round_trip_to_original_parquet_bytes() {
    ...
    for add in &adds {
        ...
        let mut chunks: Vec<sandbox_remote::ChunkRecord> = Vec::new();
        ...
        for row in &data_rows {
            if let RowBody::DataAdd {
                ...
                && file_path == &add.path
            {
                chunks.push(sandbox_remote::ChunkRecord {
                    chunk_id: *chunk_id,
...
async fn open_errors_when_property_is_corrupted_uuid() {
    ...
    let _ = DeltaTable::try_from_url(url)
        ...
        .unwrap()
        .create()
        .with_columns(sandbox_remote::delta_columns())
        .with_partition_columns(sandbox_remote::partition_columns())
    ...
    match Remote::open(&bad_path).await {
        Err(sandbox_remote::RemoteError::InvalidRemote(msg)) => {
...
async fn idempotent_re_push_after_simulated_post_commit_crash_writes_completed() {
    ...
    for r in &log_after {
        if r.txn_seq == 1
            ...
            && latest
                .map(|prev: &sandbox_steward::ControlRecord| r.ts_micros > prev.ts_micros)
                .unwrap_or(true)
        {
            latest = Some(r);
        }
