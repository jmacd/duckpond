...
pond cat /sensors/readings.csv --query "SELECT * FROM source WHERE temperature > 23"

# Query: Aggregate
echo ""
echo "=== Query: AVG temperature ==="
pond cat /sensors/readings.csv --query "SELECT AVG(temperature) as avg_temp, AVG(humidity) as avg_humidity FROM source"

# Query: Transform
echo ""
echo "=== Query: Temperature in Fahrenheit ==="
pond cat /sensors/readings.csv --query "SELECT timestamp, temperature * 9/5 + 32 as temp_f FROM source"
