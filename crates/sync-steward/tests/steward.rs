...
use sync_steward::{CommitKind, RecordKind, Steward, StewardError, StewardOptions, verify_local};
...
fn opts_merkle() -> StewardOptions {
    StewardOptions {
        checksum_strategy: Arc::new(sync_store::checksum::Merkle::new()),
        ..Default::default()
    }
}
...
fn opts_homomorphic() -> StewardOptions {
    StewardOptions {
        checksum_strategy: Arc::new(sync_store::checksum::Homomorphic::new()),
        ..Default::default()
    }
}
...
async fn legacy_pond_without_store_id_errors_on_open() {
    ...
    let _ = sync_store::Store::create(&pond.join("data")).await.unwrap();
    let _ = sync_steward::control_table::ControlTable::create(&pond.join("control"))
        .await
        .unwrap();
...
async fn verify_local_detects_data_drift() {
    ...
    {
        let mut store = sync_store::Store::open(dir.path().join("data"))
            .await
            .unwrap();
