...
mod excel_html_integration_tests {
    ...
    async fn test_excelhtml_hydrovu_file() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let table = provider.create_table_provider("excelhtml:///hydrovu.htm", &ctx).await?;
        
        // Register and query
        let _ = ctx.register_table("hydrovu", table)?;
        
        // Count rows
        let count_df = ctx.sql("SELECT COUNT(*) as total_rows FROM hydrovu").await?;
        let results = count_df.collect().await?;
        
        assert!(!results.is_empty());
        let row_count = results[0].column(0);
        let count = row_count.as_any().downcast_ref::<arrow::array::Int64Array>().unwrap();
