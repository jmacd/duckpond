...
mod tests {
    ...
    fn test_error_context_extension() {
        ...
        assert!(mapped.unwrap_err().to_string().contains("Failed to open /test/path"));
    }
    ...
    fn test_transaction_context() {
        ...
        assert!(mapped.is_err());
        assert!(mapped.unwrap_err().to_string().contains("Failed to begin transaction"));
    }
    ...
    fn test_common_errors() {
        ...
        assert!(err.to_string().contains("Failed to open table at version 42"));
    }
}
