...
impl BenchmarkMetrics {
    ...
    fn memory_efficiency_per_1k_rows(&self) -> Option<f64> {
        self.peak_memory_mb
            .map(|mb| mb / (self.rows as f64 / 1000.0))
    }
}
...
fn generate_test_data(rows: usize) -> Result<arrow_array::RecordBatch> {
    ...
    // Create RecordBatch
    let batch = arrow_array::RecordBatch::try_new(schema, vec![timestamp_array, value_array])?;

    debug!(
        "Generated test data: {} rows, {} columns",
        batch.num_rows(),
        batch.num_columns()
    );
    Ok(batch)
...
async fn read_and_verify_derived_data(
    ...
    let derived_matches = root
        .collect_matches("/benchmark-derived/**")
        .await
    ...
    let (read_batches, actual_row_count) =
        if let Some((first_derived_path, _)) = derived_matches.first() {
            ...
            let mut stream =
                tlogfs::execute_sql_on_file(&root, actual_path, sql_query, tx.transaction_guard()?)
                    .await
                    .map_err(|e| {
                        anyhow::anyhow!("Failed to execute SQL query on derived series: {}", e)
                    })?;
            ...
            debug!(
                "SQL-DERIVED STREAMING READ: Read {} rows in {} batches from {}",
                total_rows, batch_count, actual_path
            );

            (collected_batches, total_rows)
        } else {
            return Err(anyhow::anyhow!(
                "No files found in /benchmark-derived directory"
            ));
        };
    ...
    info!(
        "Derived data read in {:?}, {} total rows in {} batches",
        read_time,
        actual_row_count,
        read_batches.len()
    );
    ...
    if !values_match {
        ...
        debug!(
            "Expected {} timestamps, got {} timestamps",
...
async fn read_and_verify_data(
        ...
        .transact(
            ...
            |_tx, fs| {
                Box::pin(async move {
                    ...
                    let data = root
                        .read_file_path_to_vec("test_series.series")
                        .await
                        .map_err(|e| {
                            steward::StewardError::DataInit(tlogfs::TLogFSError::TinyFS(e))
                        })?;
                    ...
                    let reader_builder =
                        ParquetRecordBatchReaderBuilder::try_new(bytes).map_err(|e| {
                            steward::StewardError::DataInit(tlogfs::TLogFSError::TinyFS(
                                tinyfs::Error::Other(format!("Parquet reader error: {}", e)),
                            ))
                        })?;
                    let mut stream_reader = reader_builder.build().map_err(|e| {
                        steward::StewardError::DataInit(tlogfs::TLogFSError::TinyFS(
                    ...
                    while let Some(batch_result) = stream_reader.next() {
                        ...
                        debug!(
                            "Streamed batch {} with {} rows (total: {})",
                            batch_count,
                            collected_batches.last().unwrap().num_rows(),
                            total_rows
                        );
                    ...
                    debug!(
                        "STREAMING: Read {} rows in {} batches",
                        total_rows, batch_count
                    );

                    Ok::<(Vec<arrow_array::RecordBatch>, usize), steward::StewardError>((
                        collected_batches,
                        total_rows,
                    ))
                })
    ...
    info!(
        "Data read in {:?}, {} total rows in {} batches",
        read_time,
        actual_row_count,
        read_batches.len()
    );
    ...
    if !rows_match {
        warn!(
            "Row count mismatch: expected {}, got {}",
            expected_rows, actual_row_count
        );
    }
    if batch_count > 1 {
        debug!(
            "Multiple batches (normal for parquet): {} batches for {} rows",
            batch_count, actual_row_count
        );
    }
    ...
    if !read_batches.is_empty() && rows_match {
        ...
        if read_batches[0].num_columns() != expected_batch.num_columns() {
            warn!(
                ...
                read_batches[0].num_columns()
            );
            values_match = false;
        } else {
            // Extract all timestamps from expected data
            let expected_ts_col = expected_batch
                .column(0)
                .as_any()
                .downcast_ref::<arrow_array::Int64Array>()
                .unwrap();
            ...
            if read_timestamps != expected_timestamps {
                let missing_from_read: Vec<_> = expected_timestamps
                    .difference(&read_timestamps)
                    .take(5)
                    .collect();
                let extra_in_read: Vec<_> = read_timestamps
                    .difference(&expected_timestamps)
                    .take(5)
                    .collect();
                ...
                warn!(
                    "  Expected set size: {}, Read set size: {}",
                    expected_timestamps.len(),
                    read_timestamps.len()
                );
                values_match = false;
                ...
                debug!(
                    "✅ Timestamp verification passed: {} unique timestamps match exactly",
                    expected_timestamps.len()
                );
            }
        }
...
async fn run_benchmark_iteration(
    ...
    let ship_context = ShipContext::new(
        Some(pond_path.to_path_buf()),
        vec!["benchmark".to_string(), "debug-show".to_string()],
    );
    ...
    .await
    .map_err(|e| anyhow::anyhow!("Failed to run show command: {}", e))?;
...
fn print_streaming_results(metrics: &[BenchmarkMetrics], row_sizes: &[usize]) {
    ...
    let mut size_groups: std::collections::HashMap<usize, Vec<&BenchmarkMetrics>> =
    ...
    println!(
        "│ Dataset Size │ Write Tput  │ Orig Read   │ Derived Read│ Peak Memory │ MB per 1K   │ Verification │"
    );
    println!(
        "│              │ (rows/sec)  │ (rows/sec)  │ (rows/sec)  │ (MB)        │ rows        │              │"
    );
    println!(
        "├──────────────┼─────────────┼─────────────┼─────────────┼─────────────┼─────────────┼──────────────┤"
    );
    ...
    for &row_size in row_sizes {
        if let Some(group_metrics) = size_groups.get(&row_size) {
            ...
            let avg_write_tput = group_metrics
                .iter()
                .map(|m| m.write_throughput())
                .sum::<f64>()
                / group_metrics.len() as f64;
            let avg_read_tput = group_metrics
                .iter()
                .map(|m| m.read_throughput())
                .sum::<f64>()
                / group_metrics.len() as f64;
            let avg_derived_tput = group_metrics
                .iter()
                .map(|m| m.derived_read_throughput())
                .sum::<f64>()
                / group_metrics.len() as f64;
            let avg_memory = group_metrics
                .iter()
                .filter_map(|m| m.peak_memory_mb)
                .sum::<f64>()
                / group_metrics
                    .iter()
                    .filter(|m| m.peak_memory_mb.is_some())
                    .count()
                    .max(1) as f64;
            let avg_efficiency = group_metrics
                .iter()
                .filter_map(|m| m.memory_efficiency_per_1k_rows())
                .sum::<f64>()
                / group_metrics
                    .iter()
                    .filter(|m| m.memory_efficiency_per_1k_rows().is_some())
                    .count()
                    .max(1) as f64;
            let passed_count = group_metrics
                .iter()
                .filter(|m| m.verification_passed)
                .count();

            println!(
                "│ {:>12} │ {:>11.0} │ {:>11.0} │ {:>11.0} │ {:>11.1} │ {:>11.2} │ {:>4}/{:<7} │",
    ...
    if row_sizes.len() > 1 {
        ...
        let efficiencies: Vec<(usize, f64)> = row_sizes
            .iter()
            .filter_map(|&size| {
        ...
        if efficiencies.len() > 1 {
            let best_efficiency = efficiencies
                .iter()
                .min_by(|a, b| a.1.partial_cmp(&b.1).unwrap_or(std::cmp::Ordering::Equal));
            let worst_efficiency = efficiencies
                .iter()
            ...
            if let (Some(best), Some(worst)) = (best_efficiency, worst_efficiency) {
                ...
                println!(
                    "   • Best efficiency: {:.2} MB/1K rows at {} rows",
                    best.1,
                    format_row_count(best.0)
                );
                println!(
                    "   • Worst efficiency: {:.2} MB/1K rows at {} rows",
                    worst.1,
                    format_row_count(worst.0)
                );
                if improvement > 0.0 {
                    println!(
                        "   • Memory efficiency improves by {:.1}% with larger datasets",
                        improvement
                    );
                } else {
                    println!(
                        "   • Memory efficiency degrades by {:.1}% with larger datasets",
                        -improvement
                    );
                }
            }
        ...
        println!(
            "   • Status: {} behavior indicates {} memory usage",
            if efficiencies.windows(2).all(|w| w[1].1 <= w[0].1 * 1.1) {
                "✅ Good"
            } else {
                "⚠️  Variable"
            },
            if efficiencies.iter().all(|(_, eff)| *eff < 1.0) {
                "efficient"
            } else {
                "potentially high"
            }
...
async fn main() -> Result<()> {
    ...
    info!(
        "Starting DuckPond streaming benchmark with row sizes: {:?}, {} iterations each",
        row_sizes, args.iterations
    );
    ...
    for (size_idx, &row_count) in row_sizes.iter().enumerate() {
        ...
        for iteration in 1..=args.iterations {
            info!(
                "Running iteration {}/{} for {} rows",
                iteration, args.iterations, row_count
            );

            // Use unique pond path for each row size and iteration
            let iteration_pond_path =
                base_pond_path.with_extension(&format!("{}rows_iter{}", row_count, iteration));

            let metrics =
                run_benchmark_iteration(&iteration_pond_path, row_count, args.monitor_memory)
                    .await?;
    ...
    let failed_iterations = all_metrics
        .iter()
        .filter(|m| !m.verification_passed)
        .count();
    if failed_iterations > 0 {
        warn!(
            "{} out of {} total tests failed verification",
            failed_iterations,
            all_metrics.len()
        );
        std::process::exit(1);
