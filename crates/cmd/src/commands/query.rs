...
mod tests {
    ...
    /// Test setup helper for query command tests
    struct TestSetup {
        #[allow(dead_code)]
        temp_dir: TempDir,
        ship_context: ShipContext,
    ...
    #[tokio::test]
    async fn test_query_command_basic() -> Result<()> {
        let setup = TestSetup::new().await?;
        
        // Test basic node table query
        ...
        // Should succeed even with empty table
        assert!(result.is_ok());
    ...
    #[tokio::test]
    async fn test_query_show_command() -> Result<()> {
        let setup = TestShipEnvironment::new().await?;
        
        // Test the predefined show queries
        let result = query_show_command(&setup.ship_context, &FilesystemChoice::Data).await;
