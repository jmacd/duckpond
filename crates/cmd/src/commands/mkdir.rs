...
pub async fn mkdir_command(ship_context: &ShipContext, path: &str, create_parents: bool) -> Result<()> {
    ...
    ship.transact(
        vec!["mkdir".to_string(), path_for_closure.clone(), if create_parents { "-p".to_string() } else { "".to_string() }],
            ...
            if create_parents {
                ...
                for (i, component) in path_components.iter().enumerate() {
                    ...
                    if root.exists(&current_path).await {
                        ...
                        root.create_dir_path(&current_path).await
                            .map_err(|e| steward::StewardError::DataInit(tlogfs::TLogFSError::TinyFS(e)))?;
                    }
                }
            } else {
                ...
                if let Some(parent_pos) = path_for_closure.rfind('/') {
                    ...
                    if !root.exists(parent_path).await {
                        return Err(steward::StewardError::DataInit(
                            tlogfs::TLogFSError::TinyFS(tinyfs::Error::not_found(&format!("Parent directory '{}' does not exist", parent_path)))
                        ));
                    }
                }
                ...
                if root.exists(&path_for_closure).await {
                    return Err(steward::StewardError::DataInit(
                        tlogfs::TLogFSError::TinyFS(tinyfs::Error::already_exists(&path_for_closure))
                    ));
                }
...
mod tests {
    ...
    async fn test_mkdir_single_directory() {
        ...
        mkdir_command(&setup.ship_context, "testdir", false).await
            .expect("Failed to create directory");
        
        // Verify the directory exists
        assert!(setup.verify_directory_exists("testdir").await.expect("Failed to check directory"),
                "Directory should exist after mkdir");
        
        // Verify it's actually a directory type
        let contents = setup.list_directory_contents("/").await.expect("Failed to list root");
        assert!(contents.contains(&"/testdir".to_string()), 
    ...
    async fn test_mkdir_nested_directory_with_parents() {
        ...
        mkdir_command(&setup.ship_context, "parent/child/grandchild", true).await
            .expect("Failed to create nested directory");
        
        // Verify all levels exist
        assert!(setup.verify_directory_exists("parent").await.expect("Failed to check parent"),
                "Parent directory should exist");
        assert!(setup.verify_directory_exists("parent/child").await.expect("Failed to check child"),
                "Child directory should exist");
        assert!(setup.verify_directory_exists("parent/child/grandchild").await.expect("Failed to check grandchild"),
    ...
    async fn test_mkdir_already_exists() {
        ...
        mkdir_command(&setup.ship_context, "existingdir", false).await
        ...
        assert!(result.is_err(), "mkdir without -p should fail for existing directories");
        
        // Try to create the same directory with -p - should succeed (idempotent behavior)  
        mkdir_command(&setup.ship_context, "existingdir", true).await
            .expect("mkdir -p should succeed for existing directories");
        
        // Verify directory still exists
        assert!(setup.verify_directory_exists("existingdir").await.expect("Failed to check directory"),
    ...
    async fn test_mkdir_with_existing_file_conflict() {
        ...
        // Create a file first
        setup.create_pond_file("conflictpath", "file content").await
        ...
        assert!(result.is_err(), "mkdir should fail when path already exists as file");
        
        // Verify the original file still exists
        assert!(!setup.verify_directory_exists("conflictpath").await.expect("Failed to check path"),
    ...
    async fn test_mkdir_multiple_directories() {
        ...
        mkdir_command(&setup.ship_context, "dir1", false).await
            .expect("Failed to create dir1");
        mkdir_command(&setup.ship_context, "dir2", false).await
            .expect("Failed to create dir2");
        mkdir_command(&setup.ship_context, "dir1/subdir", false).await
            .expect("Failed to create subdir");  // This works because dir1 already exists
        
        // Verify all directories exist
        assert!(setup.verify_directory_exists("dir1").await.expect("Failed to check dir1"),
                "dir1 should exist");
        assert!(setup.verify_directory_exists("dir2").await.expect("Failed to check dir2"),
                "dir2 should exist");
        assert!(setup.verify_directory_exists("dir1/subdir").await.expect("Failed to check subdir"),
                "dir1/subdir should exist");
        
        // Verify the directory structure by listing contents
        let root_contents = setup.list_directory_contents("/").await.expect("Failed to list root");
        assert!(root_contents.contains(&"/dir1".to_string()), "Root should contain dir1");
        assert!(root_contents.contains(&"/dir2".to_string()), "Root should contain dir2");
        
        let dir1_contents = setup.list_directory_contents("dir1").await.expect("Failed to list dir1");
        assert!(dir1_contents.contains(&"/dir1/subdir".to_string()), "dir1 should contain subdir");
    }
    ...
    async fn test_mkdir_absolute_vs_relative_paths() {
        ...
        mkdir_command(&setup.ship_context, "/absolute/path", true).await
            .expect("Failed to create absolute path");
        
        // Create directory with relative path (needs -p for nested creation)
        mkdir_command(&setup.ship_context, "relative/path", true).await
            .expect("Failed to create relative path");
        
        // Both should exist
        assert!(setup.verify_directory_exists("/absolute/path").await.expect("Failed to check absolute"),
                "Absolute path should exist");
        assert!(setup.verify_directory_exists("relative/path").await.expect("Failed to check relative"),
    ...
    async fn test_mkdir_special_characters() {
        ...
        mkdir_command(&setup.ship_context, "dir-with-dashes", false).await
            .expect("Failed to create dir with dashes");
        mkdir_command(&setup.ship_context, "dir_with_underscores", false).await
            .expect("Failed to create dir with underscores");
        mkdir_command(&setup.ship_context, "dir.with.dots", false).await
            .expect("Failed to create dir with dots");
        
        // Verify all exist
        assert!(setup.verify_directory_exists("dir-with-dashes").await.expect("Failed to check dashes"),
                "Directory with dashes should exist");
        assert!(setup.verify_directory_exists("dir_with_underscores").await.expect("Failed to check underscores"),
                "Directory with underscores should exist");
        assert!(setup.verify_directory_exists("dir.with.dots").await.expect("Failed to check dots"),
    ...
    async fn test_mkdir_transaction_rollback_on_error() {
        ...
        setup.create_pond_file("conflict", "content").await
        ...
        assert!(!setup.verify_directory_exists("conflict").await.expect("Failed to check conflict"),
        ...
        let contents = setup.list_directory_contents("/").await.expect("Failed to list root");
        assert!(contents.contains(&"/conflict".to_string()), 
