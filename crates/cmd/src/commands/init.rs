...
async fn init_from_backup(ship_context: &ShipContext, init_config: InitConfig) -> Result<()> {
    ...
    for txn_seq in &transactions {
        ...
        remote::factory::apply_parquet_files_from_remote(&remote_table, &mut local_table, &pond_path, *txn_seq)
            .await
    ...
    let large_file_count = remote::factory::restore_large_files_from_remote(&remote_table, &pond_path)
        .await
