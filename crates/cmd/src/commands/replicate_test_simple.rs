...
async fn test_pond_structural_statistics() -> Result<()> {
    ...
    copy_command(&ship_context, &sources1, "/data/file1.txt", "data", &CopyOptions::default()).await?;
    copy_command(&ship_context, &sources2, "/tables/table1.parquet", "table", &CopyOptions::default()).await?;
