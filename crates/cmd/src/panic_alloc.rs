...
unsafe impl GlobalAlloc for PanicOnLargeAlloc {
    unsafe fn alloc(&self, layout: Layout) -> *mut u8 {
        if layout.size() > self.max_size {
            panic!("Allocation of {} bytes exceeds limit of {} bytes", layout.size(), self.max_size);
        }
    ...
    unsafe fn alloc_zeroed(&self, layout: Layout) -> *mut u8 {
        if layout.size() > self.max_size {
            panic!("Zeroed allocation of {} bytes exceeds limit of {} bytes", layout.size(), self.max_size);
        }
