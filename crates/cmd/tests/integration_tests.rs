...
fn test_pond_init_and_show() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(!init_again_output.status.success(), "Init should fail when pond already exists");
...
fn test_show_without_init() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Should fail with error
    assert!(!output.status.success(), "Show should fail when pond doesn't exist");
...
fn test_missing_pond_env() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Should fail with error
    assert!(!output.status.success(), "Init should fail when POND env var is not set");
    let stderr = String::from_utf8_lossy(&output.stderr);
