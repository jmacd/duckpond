...
async fn push_records_post_push_pending_then_completed() {
    ...
    let pending: Vec<_> = log
        .iter()
        .filter(|r| r.txn_seq == 1 && r.record_kind == sync_steward::RecordKind::PostPushPending)
        .collect();
    let completed: Vec<_> = log
        .iter()
        .filter(|r| r.txn_seq == 1 && r.record_kind == sync_steward::RecordKind::PostPushCompleted)
        .collect();
...
async fn push_errors_on_store_id_mismatch() {
    ...
    match remote.push(&mut steward, 1).await {
        Err(sync_remote::RemoteError::StoreIdMismatch {
            remote: r,
            steward: s,
        }) => {
...
async fn push_errors_on_missing_data_committed() {
    ...
    match remote.push(&mut steward, 1).await {
        Err(sync_remote::RemoteError::NoSuchCommit(seq)) => assert_eq!(seq, 1),
...
async fn push_errors_on_noop_compact_seq() {
    ...
    match remote.push(&mut steward, outcome.txn_seq).await {
        Err(sync_remote::RemoteError::NoSuchCommit(seq)) => {
