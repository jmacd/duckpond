...
use tlogfs::remote_factory::detect_changes_from_delta_log;
...
async fn test_detect_changes_from_commit() -> Result<(), TLogFSError> {
    ...
    let mut persistence = OpLogPersistence::create(&pond_path).await?;
    ...
    {
        let tx = persistence.begin(2).await?;
...
async fn test_detect_changes_multiple_commits() -> Result<(), TLogFSError> {
    ...
    {
        let tx = persistence.begin(3).await?;
        let root = tx.root().await
            .map_err(|e| TLogFSError::TinyFS(e))?;
