...
pub fn create_temporal_predicate(
    ...
    Box::new(move |row_group_metadata| {
        ...
        if let Some(timestamp_col) = row_group_metadata.columns()
            .iter()
...
mod tests {
    use super::*;
    use arrow::array::{TimestampSecondArray, Int64Builder};
    ...
    fn test_temporal_filter_timestamp_seconds() {
        ...
        let batch = RecordBatch::try_new(
            schema,
            vec![Arc::new(timestamp_data), Arc::new(value_data)]
        ).unwrap();
        ...
        let filtered_values = filtered_batch.column(1)
            .as_any()
            .downcast_ref::<Int64Array>()
            .unwrap();
