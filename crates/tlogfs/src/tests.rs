...
use crate::hostmount::HostmountDirectory;
...
async fn test_hostmount_end_to_end_traversal() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let persistence = OpLogPersistence::create(&store_path_str).await?;

    // Initialize root directory first
    let guard = persistence.begin().await?;
    guard.initialize_root_directory().await?;
    ...
    let _hostmount_node_id = guard
        .create_dynamic_directory(root_node_id, "mnt".to_string(), "hostmount", config_yaml)
        .await?;
    ...
    // For this test, we will use the hostmount directory directly
    let hostmount_config = HostmountConfig {
        directory: host_path.clone(),
    };
    ...
    while let Some(entry) = entries_stream.next().await {
        ...
        if let NodeType::Directory(_) = &node.node_type {
            // Traverse subdir
            if name == "subdir" {
                let subdir = crate::hostmount::HostmountDirectory::new(HostmountConfig {
                    directory: host_path.join("subdir"),
                });
                ...
                while let Some(sub_entry) = sub_entries.next().await {
                    ...
                    if let NodeType::File(file_handle) = &sub_node.node_type {
                        ...
                        assert!(
                            content.contains("nested file")
                                || content.contains("More nested content")
                                || content.contains("Hello from nested file!")
                        );
                    }
                }
...
// // Helper function for tests that need both FS and direct persistence access
// async fn create_test_filesystem_with_persistence()
// -> Result<(OpLogPersistence, TempDir), TLogFSError> {
//     let temp_dir = TempDir::new().map_err(TLogFSError::Io)?;
//     let store_path = temp_dir.path().join("test_store");
//     let store_path_str = store_path.to_string_lossy().to_string();

//     diagnostics::debug!("Creating test filesystem at: {store_path}", store_path: store_path_str);

//     // Create persistence layer
//     let persistence = OpLogPersistence::new(&store_path_str).await?;

// //     // Initialize with transaction guard
// //     {
// //         let tx = persistence.begin().await?;
// // p        let root_node_id = tinyfs::NodeID::root();
// //         match tx
// //             .load_node(root_node_id, root_node_id)
// //             .await
// //         {
// //             Ok(_) => {
// //                 diagnostics::debug!("Root directory already exists");
// //                 tx.commit(None).await.map_err(|e| TLogFSError::TinyFS(e))?;
// //             }
// //             Err(tinyfs::Error::NotFound(_)) => {
// //                 diagnostics::debug!("Initializing root directory");
// //                 tx.initialize_root_directory_transactional(tx.transaction_id())
// //                     .await
// //                     .map_err(|e| TLogFSError::TinyFS(e))?;
// //                 tx.commit().await.map_err(|e| TLogFSError::TinyFS(e))?;
// //             }
// //             Err(e) => return Err(TLogFSError::TinyFS(e)),
// //         }
// //     }

// //     // Create FS with the same persistence layer
// //     let fs = tinyfs::FS::with_persistence_layer(persistence.clone())
// //         .await
// //         .map_err(|e| TLogFSError::TinyFS(e))?;

// //     diagnostics::debug!("Test filesystem with persistence access initialized");

//     Ok((fs, persistence, temp_dir))
// }
...
//     // CRITICAL: Use only the guard-based FS creation - no direct persistence layer access
//     let fs = create_oplog_fs_with_guards(&store_path_str).await?;

//     diagnostics::debug!("Test filesystem initialized with transaction guards");

//     Ok((fs, temp_dir))
// }

// async fn create_test_filesystem_with_path(store_path: &str) -> Result<FS, TLogFSError> {
//     // CRITICAL: Use only the guard-based FS creation
...
#[tokio::test]
async fn test_guard_based_factory_prevents_empty_transactions()
...
//     let _test_file = convenience::create_file_path(&working_dir, "init_test.txt", b"test").await?;
...
//     let working_dir = fs.root().await?;
//     let _file1 = convenience::create_file_path(&working_dir, "query_test1.txt", b"data1").await?;
//     let _file2 = convenience::create_file_path(&working_dir, "query_test2.txt", b"data2").await?;
...
//     let _root_file =
//         convenience::create_file_path(&working_dir, "root_file.txt", b"root content").await?;
...
//     let _marker_file = convenience::create_file_path(
//         &empty_dir,
//         "directory_marker.txt",
//         b"empty directory marker",
//     )
//     .await?;
...
//     drop(fs);
//     let fs2 =
//         create_test_filesystem_with_path(&_temp_dir.path().join("test_store").to_string_lossy())
//             .await?;
...
//     diagnostics::info!("✅ SUCCESS: Empty directory creates proper partition structure");
//     Ok(())
// }
...
//     let working_dir = fs.root().await?;
//     let file_node_path =
//         convenience::create_file_path(&working_dir, "test_file.txt", b"initial").await?;
//     fs.commit().await?;
...
//         assert!(
//             error_msg.contains("No active transaction"),
...
//     diagnostics::info!(
//         "✅ SUCCESS: async_writer correctly rejects writes without active transaction"
//     );
//     Ok(())
// }
...
//         let error_msg = error.to_string();
//         assert!(
//             error_msg.contains("already being written in this transaction"),
//             "Expected 'already being written' error, got: {}",
//             error_msg
//         );
//     }
...
//     diagnostics::info!(
//         "✅ SUCCESS: async_writer correctly prevents concurrent writes in same transaction"
//     );
//     Ok(())
// }
...
//     assert!(
//         result.is_err(),
//         "Expected recursive write to fail, but it succeeded"
//     );
...
//         assert!(
//             error_msg.contains("already being written in this transaction"),
//             "Expected recursive write prevention error, got: {}",
//             error_msg
//         );
//     }
...
//     diagnostics::info!("✅ SUCCESS: async_writer correctly prevents recursive writes");
//     Ok(())
// }
...
//     diagnostics::info!(
//         "✅ SUCCESS: async_writer state correctly resets when writer is dropped"
//     );
//     Ok(())
// }
...
//     diagnostics::info!(
//         "✅ SUCCESS: async_writer state correctly resets after successful completion"
//     );
//     Ok(())
// }
...
//         let error_msg = error.to_string();
//         assert!(
//             error_msg.contains("File is being written in active transaction"),
//             "Expected 'being written' error, got: {}",
//             error_msg
//         );
//     }
...
//     diagnostics::info!("✅ SUCCESS: async_reader correctly rejects reads during active writes");
//     Ok(())
// }
...
//         let error_msg = error.to_string();
//         assert!(
//             error_msg.contains("Transaction") && error_msg.contains("already active"),
//             "Expected 'Transaction already active' error, got: {}",
//             error_msg
//         );
//     }
...
//     diagnostics::info!("✅ SUCCESS: begin_transaction() correctly prevents double begin");
//     Ok(())
// }
...
//     let file_node_path = convenience::create_file_path_with_type(
//         &working_dir,
//         "test_table.csv",
//         test_content,
...
//         .await
//         .map_err(|e| TLogFSError::Transaction {
//             message: format!("Failed to shutdown writer: {}", e),
//         })?;
...
//     diagnostics::info!(
//         "✅ SUCCESS: Entry type FileTable preserved during async write operation"
//     );
//     Ok(())
// }
...
//         "File content should be preserved during copy"
//     );

//     diagnostics::info!(
//         "✅ SUCCESS: Copy command entry type bug scenario fixed - FileTable preserved"
//     );
//     Ok(())
// }
...
//     let working_dir = fs.root().await?;
//     let file_node_path =
//         convenience::create_file_path(&working_dir, "test.txt", b"initial content").await?;
//     let file_node = file_node_path.borrow().await.as_file()?;
...
//     assert!(
//         final_metadata.version >= 1,
//         "File should have a valid version number >= 1, but got version {}",
//         final_metadata.version
//     );

//     println!(
//         "Multiple writes in single transaction correctly replaced - version: {}",
//         final_metadata.version
...
//     assert_eq!(
//         second_read, b"final content",
//         "Multiple reads should return the same content - the final replaced version"
//     );

//     diagnostics::info!(
//         "✅ SUCCESS: Multiple writes in single transaction correctly replaced - single final version created"
//     );
//     Ok(())
// }
...
//     diagnostics::info!("Starting basic persistence test");
//     let host_path_str = host_path.display().to_string();
//     diagnostics::info!("Host path: {path}", path: host_path_str);
//     diagnostics::info!("Store path: {path}", path: store_path_str);

//     // Test: Create dynamic directory and verify it's stored
//     let (dynamic_node_id, root_node_id) = {
//         diagnostics::info!("Phase 1: Creating persistence layer and dynamic directory");

//         let persistence = OpLogPersistence::new(&store_path_str).await?;

//         // Begin transaction and initialize root directory first
//         diagnostics::info!("Beginning transaction and initializing root directory");
//         let guard = persistence.begin().await?;
//         guard
//             .initialize_root_directory_transactional(guard.transaction_id())
//             .await?;

//         let root_node_id = NodeID::root(); // Use the actual root node ID

//         let root_id_str = root_node_id.to_hex_string();
//         diagnostics::info!("Root node ID: {id}", id: root_id_str);
//         diagnostics::info!("Creating dynamic directory with name: hostmount_test");

//         // Create the dynamic directory using our persistence layer
//         let created_node_id = guard
//             .create_dynamic_directory_transactional(
//                 root_node_id,
...
//         diagnostics::info!("Created dynamic directory with node ID: {id}", id: created_id_str);

//         // CRITICAL: Commit the transaction to persist the dynamic directory
//         diagnostics::info!("Committing transaction to persist dynamic directory");
//         guard.commit_with_metadata(None).await?;
//         diagnostics::info!("✅ Transaction committed successfully");
...
//     {
//         diagnostics::info!("Phase 2: Querying for persisted dynamic directory");

//         let persistence = OpLogPersistence::new(&store_path_str).await?;

//         let node_id_str = dynamic_node_id.to_hex_string();
//         let part_id_str = root_node_id.to_hex_string();
//         diagnostics::info!("Querying with node_id: {node_id}, part_id: {part_id}",
//                                    node_id: node_id_str,
//                                    part_id: part_id_str);
...
//             diagnostics::info!("✅ Found dynamic directory with factory: {factory}", factory: factory_type);

//             assert_eq!(
//                 factory_type, "hostmount",
//                 "Factory type should be hostmount"
//             );

//             // Verify the configuration is correct
//             let recovered_config: HostmountConfig = serde_yaml::from_slice(&config_bytes)?;
//             assert_eq!(
//                 recovered_config.directory, host_path,
//                 "Host directory path should match"
//             );

//             diagnostics::info!("✅ Configuration matches expected values");
//         } else {
//             diagnostics::info!("❌ Dynamic directory not found in persistence layer");
//             return Err("Dynamic directory should be persisted but was not found".into());
//         }
//     }

//     diagnostics::info!("✅ SUCCESS: Basic dynamic directory persistence working");
//     Ok(())
// }
