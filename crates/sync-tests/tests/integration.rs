...
use sync_remote::{MaintainOptions, Remote, RemoteError, verify_against_remote};
use sync_steward::{CommitKind, PartitionChecksums, Steward, StewardOptions};
use sync_store::checksum::{Checksum, Merkle};
...
) -> std::result::Result<sync_remote::PullReport, sync_remote::RemoteError> {
...
async fn verify_invariants_at_every_step_of_a_long_sequence() {
    ...
    for i in 0..5 {
        ...
        let local_src = sync_steward::verify_local(&source).await.unwrap();
        assert!(local_src.ok, "step {}: source verify_local", i);

        let local_cons = sync_steward::verify_local(&consumer).await.unwrap();
...
async fn scripted_25_operation_stress_invariants_hold_throughout() {
    ...
    async fn check(source: &Steward, consumer: &Steward, remote: &Remote, step: usize) {
        let r = sync_steward::verify_local(source).await.unwrap();
        assert!(r.ok, "step {}: source verify_local", step);
        let r = sync_steward::verify_local(consumer).await.unwrap();
    ...
    let outcome = source.compact(None).await.unwrap();
    step += 1;
    let _ = sync_steward::verify_local(&source).await.unwrap();
    // 13: push + pull + check.
    push_all_pending(&mut source, &mut remote).await;
    let _ = remote.pull(&mut consumer).await.unwrap();
    step += 1;
