...
async fn test_deltalake_commit_and_read_same_handle() -> Result<()> {
    ...
    assert!(second_version > first_version, "Second version should be greater than first version");
    assert_eq!(table.version(), second_version, "Table should immediately reflect its own commits");
